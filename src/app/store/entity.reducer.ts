...
import { LoadingState, getCallStateAdapter } from '~store/call-state';
import { IAppEntityState } from './entity.state';

export function getEntityInitialState<
  Entity extends IEntity,
>(): IAppEntityState<Entity> {
 ...
 */
export function getEntityReducer<
...
>(
  ...
  return createReducer(
    ...
    on(
      actions.add,
      (state) => callStateAdapter.setLoading(state as any) as any,
    ),
    ...
    on(
      actions.addfailure,
      (state, action) =>
        callStateAdapter.setError(state as any, action.error) as any,
    ),
    ...
    on(actions.loadsuccess, (state, action) => ({
      ...state,
      ...callStateAdapter.setLoaded(
        adapter.upsertMany(action.data.data!, state) as TState,
      ),
